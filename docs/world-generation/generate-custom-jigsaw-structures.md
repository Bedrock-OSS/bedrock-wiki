...
I'll do the above steps with my tunnels and room entrances. For spawners I'll place a jigsaw facing upwards in the middle of the room with a target name specifying `wiki:spawners` and a target pool specifying `wiki:spawner`. Then on the zombie and skeleton spawners I'll place a block beneath them with the name field specifying `wiki:spawner` then I'll set the turns into field to `minecraft:stone_bricks`.
